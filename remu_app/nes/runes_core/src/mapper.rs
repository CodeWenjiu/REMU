            ...
            0x1 => match (self.ctl_reg >> 4) & 1 {
                0x0 => {
                    ...
                    self.chr_banks = [
                        ...
                        self.cart
                            .get_bank_mut(base + 0x1000, 0x1000, BankType::ChrRom),
                    ];
                }
                _ => {
    ...
    C: Cartridge,
{
    ...
    fn save(&self, writer: &mut dyn Write) -> bool {
        ...
        for v in self.prg_banks.iter() {
            if !save_prefix(&(v.as_ptr() as usize - prg_base as usize), 0, writer) {
                return false;
            }
        }
        for v in self.chr_banks.iter() {
            if !save_prefix(&(v.as_ptr() as usize - chr_base as usize), 0, writer) {
                return false;
            }
        }
