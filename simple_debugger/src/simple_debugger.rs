...
use state::{model::PipelineModel, States};
...
impl SimpleDebugger {
    ...
    fn state_init(cli_result: &OptionParser) -> (States, States) {
        ...
        let mut state = States::new(isa, reset_vector, PipelineModel::default()).unwrap();
        let mut state_ref = state.clone();

        if let Some(DifftestRef::BuildIn(_)) = cli_result.cli.differtest {
            state_ref = States::new(isa, reset_vector, PipelineModel::default()).unwrap();
        }
