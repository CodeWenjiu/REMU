...
- **Harness 与 Debugger 的 P、D、R**：`Debugger<P, R>` 中 P 为 `HarnessPolicy`、R 为 Ref 模拟器类型；内部为 `Harness<DutSim<P>, R>`，其中 remu_harness 提供 `type DutSim<P> = SimulatorRemu<P>`，使 P 与 DUT 类型的对应关系显式化。即 Debugger 直辖 Harness，P 为 harness 边界的 Policy（`D::Policy = P`），R 为 Ref 模拟器类型。约定：只在这一层用 P/R（或 D/R），不在此之外再引入一层 Policy 泛型。
